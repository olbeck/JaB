...
#' Bootstrap samples will retain vertex names of the input `graph` but will loose all other vertex and edge attributes.
#' @examples
#' #ADD EXAMPLE ONCE DATA IS ADDED
#' @export
ASE <- function(graph, d){

  A <- igraph::as_adjacency_matrix(graph, type = "both", sparse = F)
  e <- base::eigen(A)
  vecs <- e$vectors[, 1:d]
  vals <- e$values[1:d]
...
#'
#' @param graph An igraph object with $n$ nodes.
...
#'
#' @export
bootstrap_latent_space <- function(graph, d=2, B=1000,
                                   output.type = "igraph"){

  #if using output.type == "sparsematrix", package Matrix must be installed
  if(output.type == "sparcematrix"){
    if(is.element("Matrix", utils::installed.packages()[, "Package"])){
      stop("To use output.type = 'sparsematrix', the package 'Matrix' must be installed.")
    }
  }

  #get node names
  names <- igraph::V(graph)$name #is null if no names
  ...
  if(output.type == "matrix"){
    ...
    return(base::lapply(ret, function(x){Matrix::Matrix(x, sparse = T)} ))
  }else if(output.type == "igraph"){
    return(base::lapply(ret, function(x){
